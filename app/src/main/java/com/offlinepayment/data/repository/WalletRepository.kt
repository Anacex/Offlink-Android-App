...
class WalletRepository(
    ...
    suspend fun listWallets(): Result<List<WalletDto>> = withContext(ioDispatcher) {
        ...
        if (isOnline) {
            ...
            try {
                ...
                Result.failure(parseHttpFailure(e))
            } catch (e: Exception) {
                Result.failure(e)
            }
        } else {
    ...
    suspend fun getTransferHistory(limit: Int = 10): Result<List<WalletTransferResponse>> = safeCall {
        ensureSessionOrThrow()
        api.transferHistory(limit)
    }
    ...
    suspend fun saveLocalTransaction(transaction: LocalTransaction) {
        withContext(ioDispatcher) {
            ...
            if (ctx != null) {
                OfflineSyncScheduler.schedulePendingSync(ctx)
            }
        }
    }
    ...
    suspend fun getSyncedTransactions(statusFilter: String? = null, limit: Int = 50): Result<List<SyncedOfflineTransaction>> {
        return withContext(ioDispatcher) {
            ...
            if (!isOnline) {
                // Return cached synced transactions when offline.
                val cachedTransactions = getCachedSyncedTransactions(statusFilter, limit)
                return@withContext Result.success(cachedTransactions.take(limit))
            }
            ...
            try {
                ...
                val mappedError = parseHttpFailure(e)
                // On error, try to return cached data
                val cachedTransactions = getCachedSyncedTransactions(statusFilter, limit)
                if (cachedTransactions.isNotEmpty()) {
                    Result.success(cachedTransactions.take(limit))
                } else {
    ...
    private fun parseHttpFailure(e: HttpException): Exception {
        val errorBody = e.response()?.errorBody()?.string()
        if (e.code() == 403) {
            AccountBlockedParser.extractBlockedUserMessage(errorBody)?.let { msg ->
                return AccountBlockedException(msg)
            }
        }
